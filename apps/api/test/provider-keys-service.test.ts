  ...
  it("fails closed without a vault master secret in production", () => {
    ...
    try {
      expect(() =>
        ...
        // Our fail-closed deriveVaultKey wording (secret-vault module): the
        // production invariant is identical, the message names the accepted
        // fallback secret explicitly.
        /AGENTIC_KEY_VAULT_SECRET \(or AUTH_SESSION_SECRET\) is required/,
      );
    } finally {
