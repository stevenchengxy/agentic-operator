...
import { Empty, StatusDot } from "@/app/portal/components";
  ...
  return (
    <div style={rowStyle(col)}>
      <button onClick={() => hasIO && setOpen((o) => !o)} style={{ display: "flex", alignItems: "center", gap: 7, width: "100%", textAlign: "left", background: "none", border: "none", cursor: hasIO ? "pointer" : "default", padding: 0 }}>
        ...
        <span style={{ fontSize: 12.5, fontWeight: 600, color: "var(--text)", fontFamily: "var(--mono)" }}>{s.name}</span>
        <span style={{ fontSize: 11, color: col }}>{pending ? "运行中…" : s.ok ? "✓" : "✗"}</span>
      ...
      {s.reasoning && <div style={{ fontSize: 12, color: "var(--text-2)", marginTop: 3, lineHeight: 1.5 }}>{s.reasoning}</div>}
      {open && (
        <div style={{ marginTop: 4 }}>
          ...
          {s.summary && (<><div style={labelStyle}>结果（摘要）</div><div style={{ fontSize: 12, color: "var(--text-2)", lineHeight: 1.6, whiteSpace: "pre-wrap", background: "var(--panel-3)", borderRadius: 6, padding: "6px 8px" }}>{s.summary}</div></>)}
...
function Row({ icon, color, title, text, detail }: { icon: string; color: string; title: string; text?: string; detail?: string }) {
  const [open, setOpen] = useState(false);
  return (
    <div style={rowStyle(color)}>
      ...
      {text && <div style={{ fontSize: 12, color: "var(--text-2)", marginTop: 2, lineHeight: 1.55, whiteSpace: "pre-wrap" }}>{text}</div>}
...
function ActivityRow({ b }: { b: Block }) {
  switch (b.kind) {
    ...
    case "think": return <Row icon="💭" color="var(--border-2)" title="思考" text={b.text} />;
    case "message": return <Row icon="🗣" color="var(--signal)" title="大脑" text={b.text} />;
    case "plan": return <Row icon="🗺" color="var(--signal)" title={`规划 · ${b.agents} 个智能体`} text={b.summary} />;
    ...
    case "refine": return <Row icon="♻" color="var(--amber)" title={`精修 · ${b.action}`} text={b.critique} />;
    ...
    case "clarify": return <Row icon="❓" color="var(--amber)" title="询问用户" text={b.question} />;
    case "compaction": return <Row icon="🗜" color="var(--border-2)" title="上下文自动压缩" text={b.summary} detail={b.state || undefined} />;
    ...
    case "reflect": return <Row icon="💡" color="var(--border-2)" title="反思" text={b.text} />;
    case "subagent": return <Row icon="🧩" color="var(--signal)" title="子智能体" text={b.summary ?? b.task} />;
    case "toolnew": return <Row icon="🛠" color="var(--signal)" title={`新建工具 · ${b.name}`} text={b.desc} />;
...
export function ActivityLog({ blocks }: { blocks: Block[] }) {
  ...
  if (!steps.length) return <Empty title="还没有活动" hint="运行后，这里完整记录大脑每一步：思考、工具调用(输入/输出)、写的代码、评审与精修、沙箱结果——不遗漏" />;
