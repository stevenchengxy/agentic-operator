 ...
 *
 * Fail-closed response contract: only 2xx returns. Non-2xx responses THROW
 * with a bounded diagnostic so a declarative `type:"tool"` step fails the run
 * (never a silent ok:false "success") and the LLM tool loop receives a real
 * tool error it can self-correct from.
 */
...
export const httpFetchTool = defineTool({
  name: "http.fetch",
  description:
    "Generic JSON HTTP client for one manifest-bound public HTTPS origin. Pass { method, path, body?, query?, headers? }; path must be relative and sensitive headers/query values are supplied only through tenant-owned config references. Returns { status, ok, headers, body } for 2xx. 4xx/5xx throw a bounded error so both direct workflow steps and LLM tool loops observe a real failure.",
  output: z.object({
    status: z.number().int(),
    ok: z.boolean(),
  ...
  async handler(ctx) {
    ...
    // Fail-closed contract: a non-2xx upstream response is a REAL tool
    // failure. Throw (with a bounded diagnostic) instead of returning
    // `ok:false`, so a declarative `type: "tool"` step fails the run and the
    // LLM tool loop receives tool_result:is_error to self-correct — a 4xx/5xx
    // must never let a workflow run report success.
    if (response.status < 200 || response.status >= 300) {
      ...
      throw new Error(
        `http.fetch: ${method} ${safeDisplayUrl(response.finalUrl)} returned HTTP ${response.status}: ${diagnostic.slice(0, 500)}`,
      );
    }
