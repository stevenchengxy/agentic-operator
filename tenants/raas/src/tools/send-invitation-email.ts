/**
 * sendInvitationEmail — RAAS tenant tool (migrated from the old
 ...
 * WITHOUT throwing, so the run still completes and emits INTERVIEW_INVITATION_SENT
 * (delivery status rides the result). A real outage is reported, never thrown.
 */
    ...
    try {
      ...
      const res = await fetch(webhook, {
        ...
        body: JSON.stringify({
          ...
          subject,
          body,
          source: "raas/inviteInternalInterview",
